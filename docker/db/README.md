...
**Security → Code scanning** tab, category `image-j-cms-db`.
...
scanner can act on it without a human reading this file:
    ...
    docker/db/vex/j-cms-db.openvex.json
...
```sh
python3 tools/generate-db-vex.py > docker/db/vex/j-cms-db.openvex.json
```
...
```sh
docker build -f docker/db/Dockerfile -t j-cms-db:check .
trivy image --scanners vuln --severity HIGH,CRITICAL j-cms-db:check
```
