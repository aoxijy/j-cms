...
`target/j-cms-ssrf-pin-resolver.jar`. That jar is never committed to git (it is a build
...
Confirmed by building `target/j-cms.jar` (or the exploded `WEB-INF/classes` +
`WEB-INF/lib`) without `target/j-cms-ssrf-pin-resolver.jar` on the classpath and calling
...
(`ant pin-resolver-jar`) and copy `target/j-cms-ssrf-pin-resolver.jar` to their own
