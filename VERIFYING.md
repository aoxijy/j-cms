...
Each release attaches the built `j-cms.war` with its own build-provenance
...
- **Vulnerability posture** for the DB image is published as OpenVEX at
  `docker/db/vex/j-cms-db.openvex.json` — machine-enforced by the publish
