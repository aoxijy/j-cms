...
|----------|-------|---------|
| `ACR_REGISTRY` | Azure Container Registry hostname | `jcms.azurecr.io` |
| `ACR_APP_SERVICE_NAME` | App Service instance name | `j-cms-pilot` |
| `ACR_APP_RESOURCE_GROUP` | Azure resource group name | `j-cms-rg` |
...
3. Name: `ACR_APP_SERVICE_NAME`, Value: `j-cms-pilot`
...
```bash
az webapp log tail --name j-cms-pilot --slot staging
```
...
az webapp show --resource-group my-rg --name j-cms-pilot

# If new (bad) deployment is active, swap back
az webapp deployment slot swap --name j-cms-pilot \
