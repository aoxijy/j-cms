...
# prettier-ignore
description: J-CMS is delivered as a Java Web Application Archive (.war) and requires PostgreSQL with extensions.
---

J-CMS is delivered as a Java Web Application Archive (.war) and runs with Apache Tomcat. PostgreSQL with PostGIS extensions is required.
...
- Create the PostgreSQL database and user credentials (the default name is "j-cms" but it can be configured)
...
CMS_PATH=<empty>|${USER_HOME}/Web/j-cms|/opt/jcms
...
> **`CMS_TRUSTED_PROXIES`** — set this when J-CMS runs behind a reverse proxy, load balancer, or CDN. Without it, the client address comes from the direct connection (`getRemoteAddr()`), which behind a proxy is the *proxy's* address — so the IP firewall, rate limiting, and geo filtering all act on that one address and analytics/audit logs record it instead of the visitor. Set it to a Java regular expression matching your trusted proxy addresses (for example `10\.\d+\.\d+\.\d+` for a private-range load balancer); the real client IP is then taken from `X-Forwarded-For` **only** for requests arriving through those proxies (J-CMS delegates to Tomcat's `RemoteIpFilter`). Leave it unset when the container terminates client connections directly. Do not set it to match untrusted networks — that would let a client spoof its address and bypass the IP controls.
