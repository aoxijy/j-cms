...
- Log Analytics workspace (name: `jcms-pilot-law`)
...
let LogData = ContainerAppConsoleLogs
  | where ContainerAppName == "j-cms" or ContainerAppName matches regex "jcms.*"
...
Azure → Logic Apps → Create "When a Sentinel alert is triggered"
  → Post message to Teams channel: #j-cms-ops-alerts
...
2. SSH to App Service container:
   az webapp ssh --name j-cms-pilot --resource-group my-rg
...
5. If DOWN, check:
   ...
   - File store: ls -la /opt/jcms (should be writable)
...
4. Remount:
   umount /opt/jcms
   mount /opt/jcms  (or restart container)
5. Verify:
   touch /opt/jcms/test && rm /opt/jcms/test
...
- [ ] Sentinel parser `JCMSAppLog` created in Sentinel
