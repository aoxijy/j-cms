...
title: OAuth Setup
# prettier-ignore
description: J-CMS can be configured for OAuth users, roles, and groups
---

J-CMS has built-in user logins and management. OAuth can be configured for validating user logins instead, and the OAuth provider can also provide roles and groups to be used within J-CMS.
...
In the J-CMS Database, configure the OAuth provider:
...
UPDATE site_properties SET property_value = 'j-cms' WHERE property_name = 'oauth.clientId';
...
In the J-CMS Database, configure the roles mappings:
...
In the J-CMS Database, configure the groups mappings:
